...
| **P-03** ✅ | El árbol de códigos solo estaba desarrollado para Hard Costs | **Aceptada la propuesta**: las tres categorías restantes se siembran con un capítulo y un elemento «General» | `MA`, `ESG` y `SC` utilizables desde el primer día. **El desglose llegó** (P-45) y trajo además los códigos del cliente, así que la promesa de «sin migración» **no se cumplió**: `MA.General` es hoy `MA.MA1` y hubo que renombrar en la migración `0020`. Se renombró la fila, no se creó otra, y por eso ningún hallazgo cambió de sitio. Ver [`05`](./05-catalogos-y-taxonomias.md) §5.3 |
...
| **P-45** ✅ | *(nuevo)* Llega el árbol de CAPEX completo del cliente, en una hoja de cálculo, **con sus propios códigos** | **Se adoptan sus códigos y sus nombres**, y el árbol se **renombra en sitio** en vez de sembrar uno nuevo al lado | **6 tipos · 28 categorías · 141 objetos = 175 nodos**. La fila conserva su `id`, así que ni un solo hallazgo cambia de código: migración `0020`. Cuatro decisiones que confirmó el cliente y dos supuestas están en `tools/importar_arbol_capex.py`. Los `-` de sus listas **son «Otros»**, y al exportar se escriben otra vez como `-`, que es lo que admite el desplegable. Su categoría `SC.S04 Otros` no tenía tramo en la plantilla; **el cliente confirmó que su plantilla puede cambiar** y se le dio uno (filas 256-266), bajando Operativos e Imprevistos doce filas para que quede pegada a los demás soft costs. Ver [`05`](./05-catalogos-y-taxonomias.md) §5.3 |
...
Surgen del análisis del doc [`18`](./18-analisis-plantillas-reales.md). **Cinco de las nueve ya están
cerradas** —P-31, P-32, P-37, P-38 y ahora P-39—. Queda esto:
...
`[REQ]` **P-39 se cierra de otra manera: fuera Gotham.** La pregunta era si su
contrato de licencia permitía incrustarla en los PPTX enviados, y estaba
aplazada porque el cliente no localizaba el contrato. La decisión del cliente
—**Segoe UI, o Montserrat en su defecto**— la deja sin objeto: ya no hay ninguna
fuente comercial en el informe.

...
| `npm run test:inventario-activo` | El **inventario del activo** (§3.2 d): que el descriptivo se traiga de la documentación con lo que dice la memoria, **nazca diciendo que está pendiente de validar por el gestor técnico**, se edite y se guarde, que la casilla no deje validar un descriptivo vacío, que «pasa a CAPEX» **no cree nada al marcarla** y genere de una vez avisando del equipo cuyo capítulo no resuelve, y que una fotografía se ate al equipo que retrata | Un `.oculto-visual` —posicionado en absoluto— dentro de una tabla que se desplaza en horizontal **ensanchaba la página 61 px en móvil**: el `overflow-x` del contenedor no lo recorta porque su bloque contenedor está fuera. No se veía: la página simplemente se movía de lado |

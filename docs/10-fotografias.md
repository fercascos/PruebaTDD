...
| **URLs firmadas contra un S3 real, en un navegador** | La descarga **ya redirige** con un `302` cuando el almacén sabe firmar, y sigue sirviendo el binario cuando no —el de disco—. Lo que no se ha podido probar es el camino completo en un navegador: `moto` solo intercepta a `boto3` dentro del proceso de Python, así que el navegador seguiría el `302` a una URL de AWS inalcanzable. En particular, **el CORS del bucket queda sin verificar**; `AlmacenS3.comprobar()` avisa al arrancar si falta | Sí |

...
SISTEMAS = [
    (
        "Cimentación",
        "Se aprecian fisuras de retracción en la solera de la zona de aparcamiento, "
        "con anchura inferior a 1 mm y sin continuidad en los muros de contención. No se observan "
        "asientos diferenciales ni signos de agotamiento estructural.",
        "Estado aceptable.",
    ),
    (
        "Cubierta",
        "La impermeabilización de la cubierta invertida presenta acumulación de agua en "
        "torno a tres sumideros, con vegetación incipiente en las juntas perimetrales. La lámina "
        "supera su vida útil estimada.",
        "Se recomienda renovación integral a medio plazo.",
    ),
    (
        "Fachadas",
        "El revestimiento de panel composite muestra desprendimientos puntuales en la "
        "fachada norte y sellados degradados en los encuentros con carpintería.",
...
LINEAS = [
    cl.LineaCapex(
        ...
        "Aparcamiento",
        "Vida útil",
        "Analizar estado del subsuelo",
        "Alto",
        "Existen fisuras y grietas en la última losa de hormigón y en los muros de contención",
        "CORTO",
        Decimal("5500.00"),
    ),
    cl.LineaCapex(
        ...
        "Bajo",
        "Gasto operativo. Durante la visita se aprecia que necesita una limpieza en profundidad",
        "CORTO",
        Decimal("2300.00"),
    ),
    cl.LineaCapex(
        ...
        "Cubierta",
        "Vida útil",
        "Renovación de impermeabilización",
        "Moderado",
    ...
    cl.LineaCapex(
        ...
        "Fachadas",
        "Reparación",
        "Sellado de encuentros con carpintería",
        "Alto",
        "Se aprecian entradas de agua puntuales",
        "CORTO",
        Decimal("32970.00"),
    ),
    cl.LineaCapex(
        ...
        "Fachadas",
        "Mejora",
        "Sustitución de panel composite",
        "Bajo",
        "Mejora estética y de eficiencia energética",
        "MEJORAS",
        Decimal("294000.00"),
    ),
    cl.LineaCapex(
...
def main() -> int:
    ...
    print(
        f"   Diapositiva de sistema: {cap_sistema.caracteres_por_linea} car/línea × "
    ...
    for nombre, desc, val in SISTEMAS:
        a = evaluar(f"{desc}\n{val}", cap_sistema)
        print(
            f"   · {nombre:14s} {len(desc) + len(val):5d} car. → {a.severidad} ({a.ocupacion:.0%})"
        )
    ...
    print(
        "   Totales:  "
        + " · ".join(f"{k}={cl.formatear_importe(v)}" for k, v in layout.totales.items() if v)
    )
    ...
    print(
        f"   Diapositivas de tabla generadas: "
